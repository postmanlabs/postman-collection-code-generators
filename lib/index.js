...
function generate (collection, sdkOptions, callback) {
  let dirname = path.dirname(require.main.filename),
    ...
    resolveSources = (collection, variables) => {
      ...
      if (variables) {
        for (let i = 0; i < variables.length; i++) {
          if (variables[i].type.toLowerCase() === 'json') {
            try {
              variableLists[variables[i].source.scope] = new sdk.VariableList(null, variables.source.values);
            }
            catch (error) {
              throw error;
            }
      ...
      variableLists.collection = postmanCollection.variables || undefined;
