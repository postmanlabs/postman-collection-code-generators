...
function getSDKOptions () {
  return {
    ...
    environment: {
      ...
      type: Object,
      default: false,
      description: 'Specifies environment/config to be used while generating sdk'
    }
  };
}
...
function checkOptions (sdkOptions, callback) {
  ...
  if (sdkOptions.environment) {
    if (!sdkOptions.environment.type || !sdkOptions.environment.environment) {
      return callback(errors.INVALID_ENVIRONMENT, null);
    }
  }
 ...
 * @param {Object} [sdkOptions.environment] - Postman Environment
 * @param {String} [sdkOptions.environment.type] - Postman environment input type, 'json' or 'string'
 * @param {object|string} [sdkOptions.environment.source] - Source of postman environment, json or url
...
function generate (collection, sdkOptions, callback) {
  ...
  checkOptions(sdkOptions, (error, sdkopts) => {
    ...
    if (sdkOptions.environment) {
      if (sdkOptions.environment.type.toLowerCase() === 'json') {
        try {
          postmanVariables = sdk.PropertyList.isPropertyList(sdkOptions.environment.source) ?
            sdk.environment.source :
            new sdk.VariableList(null, sdkOptions.environment.source);
        }
        catch (excp) {
          return callback(excp, null);
        }
      }
      else if (sdkOptions.environment.type.toLowerCase() === 'string') {
        // TODO Complete this environment fetch from url
      }
      else {
        return callback(errors.INVALID_ENVIRONMENT, null);
      }
