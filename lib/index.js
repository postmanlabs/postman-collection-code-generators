const sdk = require('postman-collection'),
  ...
  resolveVariable = require('./utils').resolveVariables,
 ...
 * @param {Array.<{scope: String, type: String, source: Object}>} [sdkOptions.variables] - Array of Postman Variables
...
function generate (collection, sdkOptions, callback) {
  ...
  checkOptions(sdkOptions, (error, sdkopts) => {
    ...
    variableLists.collection = postmanCollection.variables ? postmanCollection.variables : null;

    if (sdkOptions.variables) {
      sdkOptions.variables.forEach((variable) => {
        if (variable.type.toLowerCase() === 'json') {
          try {
            variableLists[variable.scope] = sdk.PropertyList.isPropertyList(variable.source) ?
              variable.source :
              new sdk.VariableList(null, variable.source.values);
          }
          catch (excp) {
            return callback(excp, null);
          }
    ...
    generator(postmanCollection, resolveVariable(variableLists), codeOptions, (err, sdkSnippet) => {
      if (err) {
