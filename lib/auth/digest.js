 ...
 * All of the below calculations are carried out according to RFC 7616
 *
...
function digest (request, auth) {
  ...
  nonce = config.nonce || toBase64(ramdomString(8));
  ...
  ncount = config.nonceCount || '1';
  cnonce = config.clientNonce || toBase64(ramdomString(8));
  ...
  if (qop === 'auth') {
     ...
     */
    A2 = request.method + ':' + uri + ':' + hash[algorithm](request.body.toString());
  }
   ...
   *  - opaque
   */
  authHeaderValue = `Digest username="${username}", realm="${realm}", nonce="${nonce.repeat(ncount)}",  uri="${uri}",`;
