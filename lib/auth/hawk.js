...
function hawk(request, auth) {
  ...
  hash = config.hash || (request.body ? request.body.toString() : '');

  // other elements
  method = request.method;
  uri = request.url.getPathWithQuery();
  ...
  hawkNormalizedString = `${HAWK_HEADER}\n${ts}\n${nonce}\n${method}\n${uri}\n${host}\n${port}\n`;

  // adding hash if provided (currently does not support hash so by default set to null)
  hawkNormalizedString += hash + '\n';

  // adding ext if provided
  hawkNormalizedString += (ext ? ext.replace(/\\/g, '\\\\').replace(/\n/g, '\\n') : '') + '\n';

  // if app given then add, if dlg given then add too else add ''
  if (app) {
    hawkNormalizedString += app + '\n' + (dlg || '') + '\n';
  }
