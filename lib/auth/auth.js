...
function authorize (request, auth) {
  ...
  if (request.auth.type === 'apikey') {
    authorizedRequest = apiKey(request, auth);
  }
  else if (request.auth.type === 'bearer') {
    authorizedRequest = bearerToken(request, auth);
  }
  else if (request.auth.type === 'basic') {
    authorizedRequest = basic(request, auth);
  }
  else if (request.auth.type === 'digest') {
    authorizedRequest = digest(request, auth);
  }
  else if (request.auth.type === 'hawk') {
    authorizedRequest = hawk(request, auth);
  }
  else {
