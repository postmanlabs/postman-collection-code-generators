...
module.exports = {
  ...
  'INVALID_OUTPUT_FILE_PATH': (path) => {
    return new Error(`${path} is not a valid path. Please provide a valid path(absolute)`);
  },
  'UNABLE_TO_WRITE_FILE': (filePath) => {
    return new Error(`Could not write sdk to path: ${filePath}`);
  }
};
